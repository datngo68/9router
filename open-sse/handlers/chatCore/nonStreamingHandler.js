...
import { addBufferToUsage, filterUsageForFormat, applyTokenMultipliersToUsage } from "../../utils/usageTracking.js";
 ...
 */
export async function handleNonStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKeyId, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, trackDone, appendLog }) {
  trackDone();
  ...
  saveUsageStats({ provider, model, tokens: usage, connectionId, apiKeyId, endpoint: clientRawRequest?.endpoint });
  ...
  if (translatedResponse?.usage) {
    translatedResponse.usage = filterUsageForFormat(addBufferToUsage(applyTokenMultipliersToUsage(translatedResponse.usage)), sourceFormat);
  }
